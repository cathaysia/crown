/**
 * x86_64 BIGNUM converting upload, CPUID and RDTSC handling.
 *
 * TypeScript port of OpenSSL crypto/x86_64cpuid.pl.
 * Copyright 2005-2026 The OpenSSL Project Authors. All Rights Reserved.
 * Licensed under Apache License 2.0.
 *
 * Pinned to the reference configuration: $win64=0 (unix argument
 * registers), $avx=0.
 */
...
code += `#include crypto/cryptlib.h
...

{
  ...
  const lasttick = '%r8d';
  const lastdiff = '%r9d';
  // The perl reads `win64?8:-8` where win64 is a bareword (no use strict),
  // which evaluates as the truthy string 'win64', so this is always 8.
  const redzone = 8;

  code += `.globl	OPENSSL_instrument_bus
...
function genRandom(rdop: string): void {
  code += `.globl	OPENSSL_ia32_${rdop}_bytes
