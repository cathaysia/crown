...
const N: usize = 17;
const INIT_KONST: u32 = 0x6996c53a;
const KEYP: usize = 15;
const FOLDP: usize = 4;

#[rustfmt::skip]
const MULTAB: [u32; 256] = [
...
impl Sober128 {
    pub fn new(key: &[u8]) -> CryptoResult<Self> {
        ...
        st.r[1] = 1;
        for i in 2..N {
            st.r[i] = st.r[i - 1].wrapping_add(st.r[i - 2]);
        }

        for i in (0..keylen).step_by(4) {
            let k = u32::from_le_bytes(key[i..i + 4].try_into().unwrap());
            st.add_key(k);
            st.cycle();
            let nl = st.nltap();
            st.xor_nl(nl);
    ...
    pub fn set_iv(&mut self, iv: &[u8]) -> CryptoResult<()> {
        let ivlen = iv.len();
        if ivlen == 0 || (ivlen & 3) != 0 {
            return Err(CryptoError::InvalidIvSize(ivlen));
        }

        self.reload_state();

        for i in (0..ivlen).step_by(4) {
            let k = u32::from_le_bytes(iv[i..i + 4].try_into().unwrap());
            self.add_key(k);
            self.cycle();
            let nl = self.nltap();
            self.xor_nl(nl);
        ...
        // also fold in the length of the key (LTC uses ivlen here)
        self.add_key(ivlen as u32);
    ...
    fn add_key(&mut self, k: u32) {
        self.r[KEYP] = self.r[KEYP].wrapping_add(k);
    }

    fn xor_nl(&mut self, nl: u32) {
        self.r[FOLDP] ^= nl;
    }
    ...
    fn step(&mut self, z: usize) -> u32 {
        let r0 = self.r[off(z, 0)];
        self.r[off(z, 15)] ^ self.r[off(z, 4)] ^ (r0 << 8) ^ MULTAB[((r0 >> 24) & 0xFF) as usize]
    }

    fn nlfunc(&self, z: usize) -> u32 {
        let mut t = self.r[off(z, 0)].wrapping_add(self.r[off(z, 16)]);
        t ^= SBOX[((t >> 24) & 0xFF) as usize];
        t = t.rotate_right(8);
        t = (t.wrapping_add(self.r[off(z, 1)]) ^ self.konst).wrapping_add(self.r[off(z, 6)]);
        t ^= SBOX[((t >> 24) & 0xFF) as usize];
        t.wrapping_add(self.r[off(z, 13)])
    }
    ...
    fn save_state(&mut self) {
        self.init_r.copy_from_slice(&self.r);
    }

    fn reload_state(&mut self) {
        self.r.copy_from_slice(&self.init_r);
    }

    fn gen_konst(&mut self) {
        loop {
            self.cycle();
            let new_konst = self.nltap();
            if (new_konst & 0xFF000000) != 0 {
                self.konst = new_konst;
                break;
            }
        }
    }
    ...
    fn diffuse(&mut self) {
        for z in 0..N {
            let t = self.step(z);
            self.r[off(z, 0)] = t;
            let nl = self.nlfunc(z + 1);
            self.r[off(z + 1, FOLDP)] ^= nl;
        }
    }
}

#[inline]
fn off(zero: usize, i: usize) -> usize {
    (zero + i) % N
}

impl StreamCipher for Sober128 {
    fn xor_key_stream(&mut self, inout: &mut [u8]) -> CryptoResult<()> {
        let mut idx = 0;
        let mut inlen = inout.len();

        // handle any previously buffered bytes
        while self.nbuf != 0 && inlen != 0 {
            inout[idx] ^= (self.sbuf & 0xFF) as u8;
            self.sbuf >>= 8;
            self.nbuf -= 8;
            idx += 1;
            inlen -= 1;
        ...
        while inlen >= 4 {
            self.cycle();
            let t = self.nltap();
            let out_bytes = t.to_le_bytes();
            xor_bytes(&mut inout[idx..idx + 4], &out_bytes);
            idx += 4;
            inlen -= 4;
        }
