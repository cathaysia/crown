...
static SHA256_HKDF: LazyLock<[(fn() -> crate::hash::sha256::Sha256<32, false>, Vec<Vec<u8>>); 4]> =
    LazyLock::new(|| {
        [
            (
                sha256::new256,
...
static SHA1_HKDF: LazyLock<[(fn() -> crate::hash::sha1::Sha1, Vec<Vec<u8>>); 4]> =
    LazyLock::new(|| {
        [
            (
                sha1::new,
...
fn test_hkdf() {
    for tt in SHA256_HKDF.iter() {
        ...
        let mut hkdf = super::new(hash_fn, master, salt, info);
