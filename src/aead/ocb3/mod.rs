    ...
    Ocb3Impl<TAG_SIZE, NONCE_SIZE, T>
{
    ...
    fn process_nonce(&self, nonce: &[u8]) -> tinyvec::ArrayVec<[u8; MAX_BLOCK_SIZE]> {
        ...
        if nonce_len == 12 && block_size == 16 {
            ktop[..12].copy_from_slice(nonce);
            ktop[12] = 0;
            ktop[13] = 0;
            ktop[14] = 0;
            ktop[15] = 1;
        } else if nonce_len < block_size {
            ktop[..nonce_len].copy_from_slice(nonce);
            ktop[nonce_len] = 0x80;
            let bottom = (nonce_len * 8) as u8;
            ktop[block_size - 1] = bottom & 0x3f;
            ktop[block_size - 1] |= if bottom < 128 { 0x00 } else { 0x40 };
        } else {
    ...
    fn hash_aad(&self, additional_data: &[u8]) -> tinyvec::ArrayVec<[u8; MAX_BLOCK_SIZE]> {
        let block_size = self.cipher.block_size();
        ...
        for i in 0..ad_blocks {
            ...
            Self::xor_blocks(&mut block, &offset);
    ...
    for Ocb3Impl<TAG_SIZE, NONCE_SIZE, T>
{
    fn seal_in_place_separate_tag(
        ...
        let aad_hash = self.hash_aad(additional_data);
        Self::xor_blocks(&mut checksum, &aad_hash);
