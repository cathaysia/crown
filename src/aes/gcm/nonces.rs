...
pub fn new_gcm_with_counter_nonce(cipher: Block) -> Result<GCMWithCounterNonce, &'static str> {
...
pub fn new_gcm_for_tls12(cipher: Block) -> CryptoResult<GCMForTLS12> {
...
pub fn new_gcm_for_tls13(cipher: Block) -> CryptoResult<GCMForTLS13> {
    let g = GCM::new_gcm(cipher, GCM_STANDARD_NONCE_SIZE, GCM_TAG_SIZE)?;
    Ok(GCMForTLS13 {
        g,
        ready: false,
...
pub fn new_gcm_for_ssh(cipher: Block) -> CryptoResult<GCMForSSH> {
    let g = GCM::new_gcm(cipher, GCM_STANDARD_NONCE_SIZE, GCM_TAG_SIZE)?;
    Ok(GCMForSSH {
        g,
        ready: false,
