...
pub struct Cmac {
    b: Aes,
    k1: [u8; Aes::BLOCK_SIZE],
    k2: [u8; Aes::BLOCK_SIZE],
}
...
impl Cmac {
    ...
    fn derive_subkeys(&mut self) {
        ...
        self.k1.copy_from_slice(&[0; Aes::BLOCK_SIZE]);
        self.b.encrypt_block_internal(&mut self.k1);

        // Derive k1 from L
        let msb = shift_left(&mut self.k1);
        self.k1[Aes::BLOCK_SIZE - 1] ^= msb * 0b10000111;

        // Derive k2 from k1
        self.k2 = self.k1;
        let msb = shift_left(&mut self.k2);
        self.k2[Aes::BLOCK_SIZE - 1] ^= msb * 0b10000111;
    }

    /// Computes the CMAC for the given message.
    pub fn mac(&self, m: &[u8]) -> [u8; Aes::BLOCK_SIZE] {
        let mut x = [0u8; Aes::BLOCK_SIZE];
        ...
        let mut remaining = m;
        while remaining.len() >= Aes::BLOCK_SIZE {
            // XOR current block with accumulator
            copy(&mut x, &self.k1);
            xor_bytes(&mut x, &remaining[..Aes::BLOCK_SIZE]);

            if remaining.len() == Aes::BLOCK_SIZE {
            ...
            // Encrypt the result
            self.b.encrypt_block_internal(&mut x);
            remaining = &remaining[Aes::BLOCK_SIZE..];
        }
...
fn shift_left(x: &mut [u8; Aes::BLOCK_SIZE]) -> u8 {
    let mut msb = 0u8;
