...
impl MacGeneric {
    ...
    pub fn write(&mut self, mut p: &[u8]) -> usize {
        ...
        if self.offset > 0 {
            ...
            p = &p[n..];
            self.offset = 0;
            update_generic(&mut self.mac_state, &self.buffer);
        }

        let n = p.len() - (p.len() % TAG_SIZE);
        if n > 0 {
            update_generic(&mut self.mac_state, &p[..n]);
            p = &p[n..];
        }
...
fn add128(a: Uint128, b: Uint128) -> Uint128 {
    ...
    ret
    // let result =
    //     (a.lo as u128) + (a.hi as u128) * (1 << 64) + (b.lo as u128) + (b.hi as u128) * (1 << 64);
    //
    // if result >= (1u128 << 128) {
...
fn update_generic(state: &mut MacState, msg: &[u8]) {
    ...
    while !msg_slice.is_empty() {
        ...
        if msg_slice.len() >= TAG_SIZE {
            let (new_h0, carry) =
                h0.overflowing_add(u64::from_le_bytes(msg_slice[0..8].try_into().unwrap()));
            h0 = new_h0;

            let (new_h1, carry1) =
                h1.overflowing_add(u64::from_le_bytes(msg_slice[8..16].try_into().unwrap()));
            ...
            let (new_h0, carry) =
                h0.overflowing_add(u64::from_le_bytes(buf[0..8].try_into().unwrap()));
            h0 = new_h0;

            let (new_h1, carry1) =
                h1.overflowing_add(u64::from_le_bytes(buf[8..16].try_into().unwrap()));
            let (new_h1, carry2) = new_h1.overflowing_add(carry as u64);
            h1 = new_h1;
