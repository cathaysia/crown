...
fn execute_js_with_json_context(code: String, path: Option<String>) -> anyhow::Result<String> {
    ...
    let global = Source::from_bytes(include_bytes!("../jsasm/index.js"));
...
pub fn jsasm(input: TokenStream) -> TokenStream {
    ...
    match execute_js_with_json_context(code, None) {
        Ok(result) => {
...
pub fn jsasm_file(input: TokenStream) -> TokenStream {
    ...
    match execute_js_with_json_context(String::default(), Some(path)) {
        Ok(result) => {
            let expr = LitStr::new(&result, Span::call_site().into());
            quote! { #expr }.into()
        }
